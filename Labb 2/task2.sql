...
CREATE TABLE persons (
	country TEXT NOT NULL,
	personnummer TEXT NOT NULL CHECK (personnummer ~ similar_escape('[0-9]{8}-[0-9]{4}'::text, NULL::text) OR personnummer ~ similar_escape(''::text, NULL::text)),
	name TEXT NOT NULL,
...
CREATE TABLE hotels (
   ...
   ownerpersonnummer text NOT NULL CHECK (ownerpersonnummer ~ similar_escape('[0-9]{8}-[0-9]{4}'::text, NULL::text) OR ownerpersonnummer ~ similar_escape(''::text, NULL::text)),
...
CREATE OR REPLACE VIEW nextmoves AS(
   	--  NextMoves
   	-- (personcountry, personnummer, country, area, destc, desta, cost   
...
BEGIN 
	...
	--	FROM persons 
		WHERE (personnummer, country) IN ((SELECT hotels.ownerpersonnummer, hotels.ownercountry 
			...
			AND hotels.ownercountry <> new.country) s
			OR (hotels.ownerpersonnummer <> new.personnummer 
...
CREATE TRIGGER updateBudget AFTER UPDATE ON Persons
FOR EACH ROW
WHEN (new.locationarea <> old.locationarea
		OR new.locationcountry <> old.locationcountry
	)
